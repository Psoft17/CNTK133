...
function cropOpen(dataUrl,mime,mode) {
  ...
  document.getElementById('crop-title').textContent = _c.mode==='qr' ? '✂️ Chỉnh sửa ảnh QR' : '✂️ Chỉnh sửa ảnh đại diện';
...
function doMoQRTT(){
  ...
  if(nguoiNhan.qrFileID) imgWrap.innerHTML=`<img src="${nguoiNhan.qrFileID}" alt="QR" onerror="this.parentNode.innerHTML='<div class=qr-no-img>❌ Không tải được QR</div>'">`;
  else imgWrap.innerHTML=`<div class="qr-no-img">📵 ${nguoiNhan.hoTen} chưa có mã QR<br><small>Nhắc họ thêm QR trong trang Nhân Viên</small></div>`;
